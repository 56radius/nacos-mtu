  ...
  const handleImgClick = (img) => {
    setExpandedImg((prevImg) => {
      if (prevImg === img) {
        return null; // Collapse both images if clicking the expanded image
      } else if (prevImg === 'both') {
        return img; // Expand the clicked image and collapse the other
      } else {
        return 'both'; // Expand both images if neither is currently expanded
      }
    });
  };
      ...
      <div className="col-lg-6 image-container">
        <div className="team-card-container">
          <div className="team-card">
            <div className="team-img-wrapper" style={{ position: "relative" }}>  <img
                className="team-img"
                src={merit}
                style={{
                  width: "523px",
                ...
                onClick={() => setExpandedImg(expandedImg === 'merit' ? null : expandedImg === 'both' ? 'picture' : 'merit')}
              />
              <div style={{ position: "absolute", bottom: "10px", left: "10px", color: "white", backgroundColor: "rgba(0, 0, 0, 0.5)", padding: "5px" }}>  <p className="text-white name">Merit Mohammed</p>  <p className="text-white position">PRESIDENT</p>  </div>
            </div>
          </div>
        </div>
      </div>
